  ...
  <div class="container">
    ...
    <div id="sidebar">
      ...
      <div class="text-channel-category">
        ...
        <div v-if="servers">
          <div v-for="channel in current.channels" v-bind:key="channel.id">
        ...
        <div v-if="servers">
          <div v-for="channel in current.channels" v-bind:key="channel.id">
            <div v-if="channel.type == 'voice'" class="voice-channel">
              ...
              <a v-on:click="changeVoiceChannel(channel.id)">{{channel.name}}</a>
            </div>
          </div>
        </div>
      </div>
    ...
    <div id="soundboard">
      <div class="soundbutton">
        <!-- INSERT SOUNDBOARD IMAGES HERE -->
      </div>
    </div>

    <div id="people-list">
      <h2>WIP</h2>
      <button v-on:click="test">Hello there</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    test: function() {
      this.temp = !this.temp;
      if (this.temp) this.current = this.servers[1];
      else this.current = this.servers[0];
    },
    changeVoiceChannel(id) {
      ApiService.changeVoiceChannel(id);
      console.log(id);
    }
  },
  async mounted() {
    var res = await ApiService.getServers().catch(err => {
      if (err.response.status === 401) this.$router.push("/login");
    });
    this.servers = res ? res.data : undefined;
